...
namespace Voltaic.Serialization
{
    ...
    internal class MemoryDictionary<TValue>
    {
        ...
        public void Add(ReadOnlyMemory<byte> key, TValue value)
          => Add(key.Span, value);
        ...
        public bool TryAdd(ReadOnlyMemory<byte> key, TValue value)
          => TryAdd(key.Span, value);
        public bool TryAdd(ReadOnlySpan<byte> key, TValue value)
        {
            ...
            _entries[index].Key = new ReadOnlyMemory<byte>(key.ToArray());
        ...
        private int FindEntry(ReadOnlyMemory<byte> key)
          => FindEntry(key.Span);
        ...
        public bool TryGetValue(ReadOnlyMemory<byte> key, out TValue value)
        ...
        private bool KeyEquals(ReadOnlyMemory<byte> x, ReadOnlySpan<byte> y)
          => x.Span.SequenceEqual(y);

        private int GetKeyHashCode(ReadOnlyMemory<byte> obj)
          => GetKeyHashCode(obj.Span);
